...
async function readGithubText(file, loadBlob) {
  let source = file;
  if (!source || source.encoding === 'none' || typeof source.content !== 'string' || !source.content) {
    if (!source || typeof source.sha !== 'string' || typeof loadBlob !== 'function') {
      throw Object.assign(new Error('Der gemeinsame Roadbook-Speicher hat keine lesbare Datei geliefert.'), { status: 502 });
    }
    source = await loadBlob(source.sha);
  }
  if (!source || typeof source.content !== 'string' || !source.content) {
    throw Object.assign(new Error('Der gemeinsame Roadbook-Speicher hat keine lesbare Datei geliefert.'), { status: 502 });
  }
  return Buffer.from(source.content.replace(/\s/g, ''), 'base64').toString('utf8');
}
...
function createGithubStore({ token, repo = 'MrM-creates/Bike_Spain', branch = 'main', fetchImpl = fetch }) {
  ...
  return {
    ...
    async readTrip(tripId) {
      ...
      const source = await readGithubText(file, sha => request(`/git/blobs/${encodeURIComponent(sha)}`));
      return readPublishedTrip(source, tripId);
    },
    exists,
...
module.exports = { ORIGIN, SCOPES, digest, equal, random, createSealer, createGithubStore, readGithubText };
