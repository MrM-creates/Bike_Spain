...
function consentPage(ticket, error = '', writable = true, restart = '') {
...
function createAuthProvider({ sealer, store, origin, pin, reportAuthEvent = event => console.warn('roadbook_auth', event) }) {
  const resource = `${origin}/mcp`;
  const consentPolicy = (res, redirectUri) => {
    // Chromium also checks the eventual 303 destination against form-action.
    // Permit only this registered callback, never arbitrary external form targets.
    if (!callbackAllowed(redirectUri)) throw new InvalidClientMetadataError('Ungültige Rücksprungadresse.');
    res.setHeader('Content-Security-Policy', String(res.getHeader('Content-Security-Policy'))
      .replace("form-action 'self'", `form-action 'self' ${new URL(redirectUri).href}`));
  };
  ...
  const provider = {
    ...
    async authorize(client, params, res) {
      ...
      consentPolicy(res, params.redirectUri);
      const csrf = random();
    ...
    async consent(req, res) {
      ...
      consentPolicy(res, ticket.data.redirectUri);
      ...
      const reason = req.headers.origin !== origin
        ? (req.headers.origin === 'null' ? 'origin_null' : req.headers.origin ? 'origin_mismatch' : 'origin_missing')
        : !cookie ? 'cookie_missing' : !equal(cookie, ticket.data.csrf) ? 'cookie_mismatch' : '';
      if (reason) {
        // Record categories only: never PINs, cookies, OAuth values or request URLs.
        reportAuthEvent({ event: 'consent_rejected', reason });
        const data = ticket.data;
        const restart = '/authorize?' + new URLSearchParams({
          response_type: 'code', client_id: data.clientId, redirect_uri: data.redirectUri,
          scope: data.scopes.join(' '), code_challenge: data.challenge,
          code_challenge_method: 'S256', resource: data.resource,
          ...(data.state ? { state: data.state } : {})
        });
        return res.status(403).type('html').send(consentPage('',
          'Das Formular passt nicht mehr zur Anmeldung in diesem Browser. Starte die Anmeldung hier neu und gib danach deine PIN ein.',
          data.scopes.includes('roadbook:write'), restart));
      }
