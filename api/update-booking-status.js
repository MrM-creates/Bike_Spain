...
module.exports = async (request, response) => {
  ...
  try {
    ...
    const allowed = payload.action === 'authorize' ? ['action', 'secret'] : checking ? ['action', 'secret', 'tripId', 'stayId'] : ['tripId', 'stayId', 'booking', 'expectedRevision', 'secret'];
    ...
    if (typeof payload.tripId !== 'string' || typeof payload.stayId !== 'string' || !payload.stayId ||
        (!checking && (!['open', 'asked', 'booked'].includes(payload.booking) || !/^[a-f0-9]{64}$/.test(payload.expectedRevision || '')))) fail(400, 'Die Unterkunftsangaben sind unvollständig. Bitte den Reiseplan aktualisieren.');
    ...
    if (checking) {
      ...
      return json(response, 200, { ok: true, tripId: payload.tripId, stayId: payload.stayId,
        ...bookingInfo(matches[0].id, matches[0].stay, snapshot.publishedVersion), version: snapshot.publishedVersion });
    }
    ...
    return json(response, 200, { ok: true, tripId: payload.tripId, stayId: payload.stayId,
      booking: result.booking, bookingRevision: result.bookingRevision, bookingContext: result.bookingContext, version: snapshot.publishedVersion, delivery: 'deployment-pending' });
  } catch (error) {
