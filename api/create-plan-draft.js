...
const routeSchema = {
  type: "object",
  additionalProperties: false,
  required: ["summary", "decision", "replaceFromDay", "replaceCount", "days", "openItems", "sourceChecks"],
  properties: {
    ...
    sourceChecks: { type: "array", items: { type: "object", additionalProperties: false, required: ["day", "officialTitle", "officialUrl", "motorcycleTitle", "motorcycleUrl", "routingEvidence", "checkedAt", "warnings"], properties: {
      day: { type: "integer" }, officialTitle: { type: "string" }, officialUrl: { type: "string" }, motorcycleTitle: { type: "string" }, motorcycleUrl: { type: "string" }, routingEvidence: { type: "string" }, checkedAt: { type: "string" }, warnings: { type: "array", items: { type: "string" } }
    } } }
  }
};
...
const sourceCheckIssue = (plan, days, startIndex, endIndex) => {
  const checks = Array.isArray(plan?.sourceChecks) ? plan.sourceChecks : [];
  for (let index = startIndex; index < endIndex; index += 1) {
    if (days[index]?.rest) continue;
    const dayNumber = index + 1;
    const check = checks.find((item) => Number(item?.day) === dayNumber);
    if (!check) return `Für Fahrtag ${dayNumber} fehlt der vorgeschriebene Quellen-Quercheck.`;
    const officialUrl = cleanText(check.officialUrl, 1000);
    const motorcycleUrl = cleanText(check.motorcycleUrl, 1000);
    if (!cleanText(check.officialTitle, 300) || !/^https:\/\//i.test(officialUrl)) return `Fahrtag ${dayNumber} hat keine belastbare offizielle Quelle.`;
    if (!cleanText(check.motorcycleTitle, 300) || !/^https:\/\//i.test(motorcycleUrl)) return `Fahrtag ${dayNumber} hat keine unabhängige Motorradquelle.`;
    if (officialUrl === motorcycleUrl) return `Fahrtag ${dayNumber} verwendet zweimal dieselbe Quelle.`;
    if (!cleanText(check.routingEvidence, 1000)) return `Fahrtag ${dayNumber} dokumentiert den Abgleich der Streckenführung nicht.`;
  }
  return "";
};
...
const createAccommodationPlan = async ({ draftDays, accommodationContext, routeSummary, context = normalizeTripContext({}) }) => {
  ...
  if (researchStays.length) {
    ...
    const researchResults = await Promise.all(researchChunks.map((expectedStays, chunkIndex) => createStructuredResponse({
      ...
      instructions: `Du planst nur die neuen oder unpassenden Unterkuenfte eines bereits validierten Motorrad-Roadbooks. Gib fuer jeden bereitgestellten Uebernachtungsblock genau einen Eintrag in gleicher Reihenfolge aus und verwende dafuer die bereitgestellte slotId. Suche eine konkrete erste Wahl und Alternative und liefere fuer beide einen direkten HTTPS-Link zur offiziellen Unterkunftsseite oder einer serioesen Buchungsseite. Oeffne jeden vorgeschlagenen Link vor der Ausgabe: Die Zielseite muss exakt dasselbe Hotel in derselben Stadt nennen. Tote Seiten, geparkte Domains, falsche Hotels derselben Kette, Weiterleitungen auf eine andere Unterkunft oder nur alte Broschueren sind unzulaessig. Wenn keine funktionierende offizielle Objektseite existiert, verwende eine aktuelle serioese Buchungs- oder Tourismusseite fuer genau diese Unterkunft. firstChoice und alternative enthalten ausschliesslich den offiziellen kurzen Hotelnamen, niemals Adresse, Sterne, Begruendung, Parkingtext oder andere Beschreibung. Alle Details gehoeren in note. Schreibe keine Quellenlinks, Zitate oder Markdown in Namen, note, summary oder openItems; die separaten URL-Felder enthalten die Links. Behaupte sichere Garage, Ueberwachung oder Privatparkplatz nur, wenn dies auf einer aktuellen Unterkunfts-, Buchungs- oder offiziellen Tourismusseite belegt ist; sonst als unbestaetigt und vor Buchung anzufragen kennzeichnen. Wichtig sind sichere Abstellung fuer zwei beladene Motorraeder, einfache asphaltierte Zufahrt, keine problematische Altstadt- oder ZBE-Zufahrt und moeglichst stornierbare Tarife. Verfuegbarkeit und Preis gelten immer als zu pruefen. Gib ausschliesslich das strukturierte Ergebnis aus.`,
      input: JSON.stringify({ expectedStays, routeChanges: routeSummary, accommodationPreferences: context.preferences })
    })));
  ...
  try {
    ...
    if (payload.stage === "verify-route") {
      ...
      const verifiedPlan = {
        ...
        sourceChecks: verifiedChunks.flatMap((plan) => plan.sourceChecks || [])
      };
      ...
      if (singleStage) {
        ...
        if (routeStyleOnly && candidate.routeStyle && verified.routeStyle !== candidate.routeStyle) {
      ...
      const sourceIssue = sourceCheckIssue(verifiedPlan, verifiedDays, startIndex, endIndex);
      if (sourceIssue) throw new Error(`Die automatische Routenprüfung hat den Quellen-Quercheck nicht vollständig dokumentiert: ${sourceIssue}`);
