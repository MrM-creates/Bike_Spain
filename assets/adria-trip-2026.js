(function (root) {
  ...
  const snapshot = {
    ...
    trip: {
      ...
      planningProfile: {
        ...
        officialSources: [
          "HAK – kroatische Strassen-, Wind-, Baustellen- und Grenzlage",
          "promet.si – slowenische Verkehrsinformationen",
          "ASFINAG und österreichische Landesverkehrsdienste",
          "Montenegrinische Regierung und Automobilverband AMSCG",
          "ASTRA und TCS-Verkehrsinformationen"
        ],
        motorcycleSources: [
          "Kroatische Tourismuszentrale – offizieller Motorradführer",
          "aktuelle Motorrad-Reiseportale zur Adriaküste",
          "lokale Motorradberichte und belastbare aktuelle Fahrerbewertungen"
        ],
        seasonalRisks: [
      ...
      fixPoints: [
        { id: "fix_adria_start", kind: "start", title: "Start in Berikon", place: "Berikon", startsAt: "2026-09-24T08:00:00+02:00", source: "draft" },
        { id: "fix_adria_end", kind: "end", title: "Rückkehr nach Berikon", place: "Berikon", startsAt: "2026-10-23T18:00:00+02:00", source: "draft" }
      ],
      narrativeSegments: [
  ...
  root.MotorcycleTripCatalog = Object.freeze({
    ...
    list() {
      const builtIns = [
        { id: "trip_spanien_2026", name: "Spanien 2026", startDate: "2026-09-24", endDate: "2026-10-23", status: "published" },
        { id: "trip_adria_2026", name: "Adria & Balkan 2026", startDate: "2026-09-24", endDate: "2026-10-23", status: "draft" }
      ];
      return [...builtIns, ...Object.values(storedTrips()).filter((item) => item?.trip?.id && !builtIns.some((trip) => trip.id === item.trip.id)).map(tripSummary)];
    },
    getSnapshot(id, publishedSpainSnapshot) {
      const stored = storedTrips()[id];
      const storedIsCurrent = id !== "trip_adria_2026" || stored?.trip?.dataVersion === snapshot.trip.dataVersion;
      if (stored && storedIsCurrent) {
        const value = clone(stored);
        if (id === "trip_adria_2026") {
          value.trip = { ...value.trip, planningProfile: clone(snapshot.trip.planningProfile) };
        }
        return value;
      }
      return clone(id === "trip_adria_2026" ? snapshot : publishedSpainSnapshot);
    },
    saveSnapshot(value) {
      if (!value?.trip?.id || value.trip.id === "trip_spanien_2026") throw new Error("Diese Reise kann nicht als lokaler Entwurf gespeichert werden.");
    ...
    createTrip({ name, startDate, endDate, startPlace, endPlace }) {
      ...
      const newDays = Array.from({ length: duration }, (_, index) => ({
        ...
        type: "Planung offen",
        overnight: index === duration - 1 ? endPlace : startPlace,
        km: "",
        time: "",
        roads: "Noch nicht geplant",
        points: "",
        note: "Diesen Tag im Roadbook bearbeiten.",
        rest: true,
        origin: "",
        destination: "",
        waypoints: [],
      ...
      const created = {
        ...
        trip: {
          ...
          name,
          characterTitle: name,
          characterText: "Neuer Reiseentwurf. Die Tagesetappen werden im Roadbook aufgebaut.",
          startDate,
          startPlace,
          endPlace,
          timezone: "Europe/Zurich",
          ...
          participantCount: 2,
          motorcycleCount: 2,
          preferences: { routeStyle: "mixed", ridingRhythm: "relaxed", preferGoodWeather: true },
          planningProfile: {
            countries: [],
            officialSources: ["zuständige nationale Strassen- und Verkehrsbehörden", "amtliche Wetter- und Grenzinformationen"],
            motorcycleSources: ["offizielle Motorradtourismus-Routen", "aktuelle unabhängige Motorrad-Reiseberichte"],
            seasonalRisks: ["Wetter, saisonale Sperren, Baustellen, Grenzen und Zufahrtsbeschränkungen im Reisezeitraum"]
          },
          fixPoints: [
            { id: `${id}_start`, kind: "start", title: `Start in ${startPlace}`, place: startPlace, startsAt: `${startDate}T08:00:00+02:00`, source: "draft" },
            { id: `${id}_end`, kind: "end", title: `Rückkehr nach ${endPlace}`, place: endPlace, startsAt: `${endDate}T18:00:00+02:00`, source: "draft" }
          ],
