...
test('three replacement stops join arrivals, rest days and departures without changing the remaining trip',()=>{
 for(const [arrival,departure] of [[6,8],[10,13],[20,21]]){
 ...
 assert.deepEqual({...snapshot.trip,dataVersion:publicationBase.trip.dataVersion},publicationBase.trip);
 for(const d of snapshot.days.filter(d=>![6,8,10,13,20,21].includes(d.day)))assert.deepEqual(d,baseline.days.find(b=>b.id===d.id));
 for(const s of snapshot.accommodations.filter(s=>!['zadar','makarska-base','ston-return'].includes(s.id)))assert.deepEqual(s,publicationBase.accommodations.find(b=>b.id===s.id));
});
test('Makarska orientation point stays distinct from lodging and ferry navigation ends on land',()=>{
 ...
 assert.equal(new URL(feed.days[20].mapsURL).searchParams.get('destination'),'Gat Svetog Duje, Split');
